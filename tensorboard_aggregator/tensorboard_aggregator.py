...
def extract(dpath, subpath):
    scalar_accumulators = [EventAccumulator(str(dpath / dname / subpath)).Reload().scalars
                           ...
                           if dname != FOLDER_NAME and os.path.exists(dpath / dname / subpath)]
...
def console_entry_point():
    ...
    subpaths = [i for i in subpaths if os.path.exists(i)]

    if len(subpaths) == 0:
        raise argparse.ArgumentTypeError("No data found in {} matching {}".format(path, args.subpaths))
